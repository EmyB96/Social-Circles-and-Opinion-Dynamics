...
class GUI:
    ...
    def stepModel(self):
        if self.stop(self.currentStep): #EB added 6/23/2021
            self.running = False  # HS 06/29/2020
            self.setStatusStr("Steady-state is reached")
    ...
    def drawModel(self):
        ...
        #     self.modelFigure = plt.figure(1, figsize=(9.8, 7.8)) #SM 3/26/2020
        # else:
        self.modelFigure = plt.figure(1, figsize=(9.5, 7.8)) #EB added figsize 6/7/2021

        self.modelDrawFunc()
        self.modelFigure.canvas.manager.window.update()
        #plt.show() # bug fix by Hiroki Sayama in 2016 #SM 3/26/2020
        #self.modelFigure.show()

    def plotAverageOpinion(self): #EB added 6/21/2021
        plt.ion() #SM 3/26/2020
        # if self.averageOpinionPlot == None or self.averageOpinionPlot.canvas.manager.window == None:
        #     self.averageOpinionPlot = plt.figure(2, figsize=(9.8, 7.8)) #SM 3/26/2020
        # else:
        self.averageOpinionPlot = plt.figure(2, figsize=(9.5, 7.8)) #EB added figsize 6/7/2021

        currentStep = self.currentStep
        self.averageOpinionPlotFunc(currentStep)
        self.averageOpinionPlot.canvas.manager.window.update()
        #plt.show() # bug fix by Hiroki Sayama in 2016 #SM 3/26/2020
        #self.modelFigure.show()
    ...
    def start(self,func=[]):
        if len(func)==5:
            ...
            self.averageOpinionPlotFunc = func[2] #EB added 6/21/2021
            self.stop = func[3] #EB added 6/23/2021
            self.modelStepFunc = func[4]
            ...
            self.drawModel()
            self.plotAverageOpinion()
        self.buttonRun.invoke() #automatically push run button so that the simulation can be repeated several times in a loop (comment out if the 'settings' window is needed) EB 6/24/2021
